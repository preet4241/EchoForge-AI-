...
from keyboards import (
    ...
    get_payment_cancel_panel, get_payment_verification_panel, get_payment_settings_panel, get_qr_management_panel,
        ...
        for new_member in message.new_chat_members:
            if new_member.id == bot_user.id:
                ...
                await message.reply(
                    ...
                    f"🏷️ **{chat_info.type.name.title()}:** {chat_info.title}\n"
                    f"🆔 **ID:** `{chat_info.id}`\n"
...
async def connect_channel_command(client: Client, message: Message):
    ...
    try:
        # Get current chat info first
        chat_info = message.chat
        
        # Debug: Print chat type for debugging
        print(f"Connect command received - Chat type: {chat_info.type}, Chat ID: {chat_info.id}")
        
        # Check if it's a group or channel - Fix: Handle both uppercase and lowercase
        chat_type = chat_info.type.name.lower()
        if chat_type not in ["group", "supergroup", "channel"]:
            await message.reply(f"❌ यह command केवल groups/channels में use करें!\nCurrent chat type: {chat_info.type.name}")
            return
        ...
        if message.from_user:
            ...
            # Verify owner directly if we have user info
            if user_id != OWNER_ID:
                await message.reply(f"❌ केवल Owner ही इस command का use कर सकते हैं!\nआपका ID: {user_id}\nOwner ID: {OWNER_ID}")
                return
        else:
            # If no user (channel post), we assume it's posted by someone with admin rights
            # For simplicity, we'll allow this and use OWNER_ID (since only channel admins can post as channel)
            print(f"Channel command detected - assuming posted by channel admin")
            user_id = OWNER_ID
        
        # Check if this channel is already connected
        if connected_channel_id == str(chat_info.id):
            await message.reply(
                f"✅ **यह Channel पहले से Connected है!** 🎉\n\n"
                f"🏷️ **{chat_info.type.name.title()}:** {chat_info.title}\n"
                f"🆔 **ID:** `{chat_info.id}`\n"
                f"👥 **Members:** {chat_info.members_count or 'Unknown'}\n\n"
                f"📋 **Current Status:**\n"
                f"🔹 Notifications: ✅ Active\n"
                f"🔹 Database Backups: ✅ Running\n"
                f"🔹 Payment Alerts: ✅ Enabled\n\n"
                f"🚀 **Channel सफलतापूर्वक connected है!**"
            )
            return
...
async def callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    # User callbacks
        ...
        user = get_user_from_db(user_id)
        await callback_query.edit_message_text(
            f"🌟 **Wellcome back** {callback_query.from_user.first_name}! 🌟\n\n"
        ...
        try:
            ...
            if summary:
                ...
                transaction_text = (
                    f"📊 **Total Transactions Summary**\n\n"
                    f"📋 **Total transactions:** Error\n\n"
                    f"💰 **Total free credits:** Error\n"
                    f"💳 **Total credit buy:** Error\n"
                    f"👥 **Total referral credit:** Error\n\n"
                    f"💎 **Total credit earn:** Error\n"
                    f"📉 **Total credit used:** Error\n"
                    f"💰 **Remaining credit:** {user.credits:.2f}\n\n"
    ...
    # Transaction History Handlers
        ...
        await callback_query.edit_message_text(
            ...
            "━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "📝 **aap kya problem face kar rahe hai?**\n\n"
            "🔧 **aap लिख sakte hai:**\n"
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("❌ Cancel", callback_data="back_to_user")]
            ])
        )
        ...
        await callback_query.edit_message_text(
            ...
            f"💸 **Per Word Cost:** {current_tts_charge} credits\n"
            f"🎁 **Free Credits:** Short Link\n"
            f"👬 **Referral System:** Reffer User Frnds"
    ...
    # Language selection callbacks
        ...
        if user_id == OWNER_ID:
            ...
            await callback_query.edit_message_text(
                ...
                "Kripaya अपना text bheje (Maximum 3000 characters):\n\n"
                "💰 **Charges:** {current_tts_charge} credits per word",
                reply_markup=get_back_to_user()
            )
...
async def handle_text(client: Client, message: Message):
    ...
    elif user_state_data == UserState.WAITING_LINK_TIMEOUT and user_id == OWNER_ID:
        try:
            timeout_minutes = float(message.text.strip())
            if timeout_minutes <= 0:
                await message.reply("❌ Timeout 0 se ज्yaदा होनी चाहिए!")
                user_states.pop(user_id, None)
                return

            update_setting("link_timeout_minutes", timeout_minutes, "Link timeout duration in minutes")
            await message.reply(
                f"✅ **Link Timeout Updated!**\n\n"
                f"ab links {timeout_minutes} minutes तक valid रहेंगे.",
                reply_markup=get_back_to_owner()
            )
        except ValueError:
            await message.reply("❌ Invalid amount! Kripaya valid number enter kare.")
    ...
    elif user_state_data == UserState.WAITING_FREE_CREDIT and user_id == OWNER_ID:
        try:
            free_credit_amount = float(message.text.strip())
            if free_credit_amount < 0:
                await message.reply("❌ Credit amount नकारात्मक नहीं हो सकती!")
                user_states.pop(user_id, None)
                return

            update_setting("free_credit_per_link", free_credit_amount, "Free credits given per link")
            await message.reply(
                f"✅ **Free Credit Updated!**\n\n"
                f"ab प्रति link {free_credit_amount} credits मिलेंगे.",
                reply_markup=get_back_to_owner()
            )
        except ValueError:
            await message.reply("❌ Invalid amount! Kripaya valid number enter kare.")
    ...
    elif user_state_data == UserState.WAITING_BUY_CREDIT_RATE and user_id == OWNER_ID:
        try:
            credit_rate = float(message.text.strip())
            if credit_rate <= 0:
                await message.reply("❌ Credit rate 0 से ज्yaadaa होनी चाहिए!")
                user_states.pop(user_id, None)
                return

            update_setting("payment_rate", credit_rate, "Credits given per rupee for payments")
            await message.reply(
                f"✅ **Buy Credit Rate Updated!**\n\n"
                f"ab ₹1 = {credit_rate} credits की rate है.",
                reply_markup=get_back_to_owner()
            )
        except ValueError:
            await message.reply("❌ Invalid amount! Kripaya valid number enter kare.")
    ...
    # Payment Tracking Handler
        ...
        try:
            ...
            # Import datetime locally to avoid scope issues
            from datetime import datetime as dt
            
            # Prepare message to send to owner
            current_time = dt.now().strftime('%d/%m/%Y at %H:%M:%S')
            owner_message = (
...
def main():
    ...
    print("Starting TTS Bot with Web Server...")

    try:
        # Start Flask web server in background thread for deployment compatibility
        from web_server import run_web_server_in_background
        web_thread = run_web_server_in_background()
        print("🌐 Web server started for deployment compatibility")
