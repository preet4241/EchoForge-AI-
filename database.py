...
class PaymentRequest(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    unique_id = Column(String, unique=True, index=True, nullable=True)  # Unique payment request ID
    user_id = Column(BigInteger, index=True)
...
class QRCodeSettings(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    qr_code_url = Column(String)  # QR code image URL (deprecated, for backward compatibility)
    qr_code_file_id = Column(String)  # QR code image file ID from Telegram
...
class MessageTracking(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    chat_id = Column(BigInteger, index=True)  # Chat where message was sent
    message_id = Column(BigInteger, index=True)  # Telegram message ID
    user_id = Column(BigInteger, index=True, nullable=True)  # User associated with message (can be None for system messages)
    message_type = Column(String, index=True)  # 'status', 'prompt', 'payment', 'menu', 'tts_result', 'error', etc.
    delete_after_seconds = Column(Integer, default=15)  # Seconds after which to delete
    created_at = Column(DateTime, default=datetime.utcnow)
    scheduled_delete_at = Column(DateTime, index=True)  # When message should be deleted
    is_deleted = Column(Boolean, default=False)
    delete_attempted_at = Column(DateTime, nullable=True)
    delete_error = Column(String, nullable=True)  # Store any deletion errors
    related_message_id = Column(BigInteger, nullable=True)  # For linking related messages (user input -> bot response)
    context = Column(String, nullable=True)  # Additional context about the message
...
def create_tables():
    ...
    try:
        # Check if bot.db file exists (for SQLite databases)
        ...
        required_tables = ['users', 'tts_requests', 'bot_settings', 'bot_status', 'message_tracking']
